    ...
    function GamesList({ items }) {
        return items.map(item => (
            <div key={item.id} className="relative group w-362px h-206px cursor-pointer flex justify-center" >
                ...
                <div className={`absolute bottom-0 font-spoqaBold text-12px w-80px h-25px -ml-60px -mb-17px flex items-center justify-center rounded-full bg-white text-gray-r888889 group-hover:text-white group-hover:shadow-plain4 ${item.color}`}>게임시작</div>
            </div>
        ));
    }

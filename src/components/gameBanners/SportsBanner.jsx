...
import CardBg from '../../images/cardBg/card_bg_medium.png'
import CardBgHighlight from '../../images/cardBg/card_bg_medium_highlight.png'
...
const SportsBanner = () => {
    ...
    function BannerList({ items }) {
        return items.map(item => (
            <div 
                key={item.id} 
                className="relative group flex w-407px h-387px object-contain border border-gray-afafaf justify-end rounded-lg shadow-plain1 transition cursor-pointer" 
            >
                <img className="z-20 object-none" src={item.background} alt="background" />
                <div className="absolute group-hover:opacity-0"><img className="rounded-lg" src={CardBg} alt="background" /></div>
                <div className="absolute opacity-0 group-hover:opacity-100"><img className="rounded-lg" src={CardBgHighlight} alt="background" /></div>

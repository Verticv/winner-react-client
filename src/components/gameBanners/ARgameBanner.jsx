...
const ARgameBanner = () => {
    ...
    return (
        <button className={isSelected ? cardSelectedClass : cardClass} onMouseEnter={() => setSelected(true)} onMouseLeave={() => setSelected(false)} >
            ...
            <div className={`absolute left-0 bottom-0 ml-80px mb-36px w-242px h-74px rounded-full flex items-center justify-center text-white font-spoqa text-30px font-bold bg-green-r7bd26a ${isSelected && "shadow-plain3" }`}>게임시작</div>
        </button>
    )
}
